      ...
      <main className="container max-w-4xl px-0 sm:px-4 py-4 md:py-8">
        ...
        {loading ? (
          ...
          <div className="bg-card sm:rounded-2xl border-y sm:border-x border-border shadow-sm divide-y divide-border">
            {notifications.map(n => (
              <div key={n._id || n.id} className={`p-4 md:p-6 flex gap-4 hover:bg-muted/20 transition-colors ${!n.read ? 'bg-emerald-50/30 dark:bg-emerald-900/10' : ''}`}>
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                      {n.title}
                      {!n.read && <span className="h-1.5 w-1.5 rounded-full bg-rose-500 inline-block shrink-0"></span>}
                    </h3>
