...
const BazaarOfferChat = () => {
  ...
  const isSeller = !!(
    currentUserId &&
    sellerIdStr &&
    currentUserId.toString() === sellerIdStr
  );
  ...
  const fetchData = async () => {
    try {
      ...
      const historyRes = await api.get(
        `/bazaar/offer/${id}${targetOfferId ? `?offerId=${targetOfferId}` : ""}`,
      );
      if (historyRes.data.success && historyRes.data.data.length > 0) {
        let selectedOffer = historyRes.data.data[0];
        if (targetOfferId) {
          const found = historyRes.data.data.find(
            (o) => o._id === targetOfferId,
          );
      ...
      if (sellerRes.data.success)
        setSellerTemplates(sellerRes.data.data.map((t) => t.text));
  ...
  const handleSendOffer = async (e) => {
    e.preventDefault();
    if (!numericInput || isNaN(numericInput) || Number(numericInput) <= 0)
      return;
  ...
  const handlePayToUnlock = async () => {
    ...
    try {
      ...
      const options = {
        ...
        currency: "INR",
        name: "Rozsewa Bazaar",
        description: `Lead Unlock Fee — ${ad?.title || "Rozsewa"}`,
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
        ...
        <button
          ...
          className="mt-3 text-slate-500 text-sm font-medium"
        >
          Go Back
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 relative">
      ...
      {toastMsg && (
        <div
          className={`fixed top-4 left-1/2 -translate-x-1/2 z-[999] px-5 py-3 rounded-2xl shadow-xl text-sm font-bold text-white max-w-xs text-center ${toastMsg.isError ? "bg-red-500" : "bg-green-500"}`}
        >
          {toastMsg.msg}
        </div>
      )}
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 z-10 shrink-0">
        <button
          onClick={() => navigate(`/bazaar/${id}`)}
          className="w-9 h-9 bg-slate-100 rounded-full flex items-center justify-center text-slate-600 shrink-0"
        >
          <ArrowLeft className="w-4 h-4" />
        </button>
        <div className="flex-1 min-w-0">
          <h1 className="font-black text-slate-900 text-sm line-clamp-1">
            {ad?.title || "Bazaar Offer"}
          </h1>
          <p className="text-xs text-blue-600 font-bold">
            Asking: ₹{ad?.price?.toLocaleString("en-IN")}
          </p>
        </div>
        {offerThread && (
          <span
            className={`text-[10px] font-black px-2 py-1 rounded-full shrink-0 ${
              offerThread.status === "deal_locked"
                ? "bg-green-100 text-green-700"
                : offerThread.status === "rejected"
      ...
      <div className="bg-amber-50 border-b border-amber-100 px-4 py-2 flex items-start gap-2 text-amber-700 shrink-0">
        ...
        <p className="text-[10px] font-semibold leading-tight">
          Seller contact is hidden until you pay the unlock fee. Only numeric
          offers and quick replies are allowed.
        </p>
      </div>
        ...
        {offerThread?.offerHistory?.length > 0 ? (
          offerThread.offerHistory.map((msg, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`px-4 py-3 rounded-2xl max-w-[75%] shadow-sm ${isMe ? "bg-blue-600 text-white rounded-br-md" : "bg-white text-slate-800 rounded-bl-md border border-slate-100"}`}
                >
                  {msg.actionType === "numeric_offer" && (
                    <div>
                      <p className="text-[10px] font-bold opacity-70 mb-0.5">
                        {isMe ? "Your Price Offer" : "Offer Received"}
                      </p>
                      <p className="text-2xl font-black tracking-tight">
                        ₹
                        {Number(msg.numericAmount || 0).toLocaleString("en-IN")}
                      </p>
                    </div>
                  )}
                  ...
                  <p
                    className={`text-[10px] mt-1 opacity-60 ${isMe ? "text-right" : ""}`}
                  >
          ...
          <div className="text-center py-10">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {isSeller
                ? "Waiting for a buyer to make an offer."
                : "Start the conversation with an offer or quick reply below."}
            </p>
          </div>
        )}
      </div>
          ...
          <div className="space-y-2">
            <div className="bg-green-50 rounded-2xl p-3 border border-green-100 text-center">
              <p className="text-xs font-bold text-green-700">
                🔒 Deal Locked at ₹
                {Number(offerThread?.currentOfferAmount || 0).toLocaleString(
                  "en-IN",
                )}
              </p>
            </div>
            ...
            {isContactUnlocked && contactDetails && !isSeller ? (
                ...
                {(contactDetails.exactAddress || contactDetails.areaName) && (
                  <a
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                      [
                        contactDetails.houseNumber,
                        contactDetails.exactAddress,
                        contactDetails.areaName,
                        contactDetails.city,
                      ]
                        .filter(Boolean)
                        .join(", "),
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-slate-500 font-semibold">
                        Pickup Location{" "}
                        <span className="text-blue-500">(Tap to map)</span>
                      </p>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-slate-900">
                      Reveal Seller's Contact
                    </p>
                    <p className="text-[10px] text-slate-500">
                      Pay ₹{unlockStatus.fee} via wallet to see phone & address
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="bg-green-50 rounded-2xl p-3 border border-green-100 text-center">
                <p className="text-xs font-bold text-green-700">
                  ✅ Deal locked! Buyer will pay the unlock fee to view your
                  contact.
                </p>
              </div>
            )}
            ...
            {!offerThread && (
              <div className="bg-blue-50 rounded-xl px-4 py-3 border border-blue-100">
                <p className="text-xs font-bold text-blue-700">
                  Enter your price offer below to start negotiation.
                </p>
                <p className="text-[10px] text-blue-500 mt-0.5">
                  Quick-reply messages unlock after you send your first price
                  offer.
                </p>
              </div>
            )}
            ...
            {offerThread?.status === "pending" && (
              <div className="bg-amber-50 rounded-xl px-4 py-3 border border-amber-100 text-center">
                ...
                <p className="text-[10px] text-amber-600 mt-0.5">
                  You can send another offer once the seller responds or
                  counters.
                </p>
              </div>
            )}
            ...
            {offerThread?.status === "countered" && (
              <div className="flex gap-2 pb-1">
                ...
                >
                  ...
                  {Number(offerThread.currentOfferAmount).toLocaleString(
                    "en-IN",
                  )}
                  )
                </button>
