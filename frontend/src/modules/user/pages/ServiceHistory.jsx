...
const ServiceHistory = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 md:pb-0">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {["all", "confirmed", "completed", "cancelled"].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`shrink-0 rounded-full px-5 py-2 text-[13px] font-bold capitalize transition-colors ${filter === f ? "bg-blue-600 text-white shadow-md border border-blue-500" : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700"
                  }`}>
                {f === "all" ? "All" : f}
              </button>
            ))}
      ...
      <main className="container max-w-2xl px-5 sm:px-8 space-y-6">

        {/* List */}
        <div className="space-y-4">
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50 dark:bg-slate-900">
              <Package className="h-12 w-12 text-slate-300 dark:text-slate-700 mb-3" />
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">No bookings found</p>
            </div>
          )}
          {filtered.map((booking, i) => (
            <motion.div key={booking.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              onClick={() => openDetails(booking)}
              className="group cursor-pointer rounded-[24px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 transition-all hover:border-blue-500/30 hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right shrink-0">
                  <p className="text-lg font-black text-slate-900 dark:text-white">₹{booking.total}</p>
                </div>
              </div>

              <div className="flex items-center gap-5 rounded-2xl bg-slate-50 dark:bg-slate-800 p-3.5 text-[13px] font-bold text-slate-700 dark:text-slate-300">
                <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-blue-500" /> {booking.date}</div>
